...
- 2026-03-17: Step 9 追加対応（ビジター概念整理・バグ修正・参加者一覧UI改善）※下記参照

---

## 追加対応（2026-03-17）

Phase 3 完了後のレビュー・動作確認で発覚した問題と、設計判断を伴う追加対応をまとめる。

### A. ビジター概念の整理（設計判断）

**背景**: もともと `isGuestVisitor`（未登録ビジター）と `isVisitor`（登録済みビジター）を区別するフラグがドメイン・API・FEに散在していた。命名が混乱しており、ビジネスロジック上も区別が曖昧だった。

**設計判断**:
- ドメインモデルは **`isVisitor` + `userId` の null 判定** で統一
  - `isVisitor=true && userId!=null` → 登録済みビジター
  - `isVisitor=true && userId=null` → 未登録ビジター（旧ゲストビジター）
- `isGuestVisitor` フラグを API レスポンス（`ParticipantItem`）から **削除**
- FE 側では表示のみ「（ゲスト）」ラベルで区別（内部ロジックは `isVisitor && !userId` で判定）

**変更ファイル（BE）**:
- `Participation.ts` — `createGuestVisitor()` → `createUnregisteredVisitor()`、`isGuestVisitor()` → `isUnregisteredVisitor()` にリネーム
- `ListParticipationsUseCase.ts` — レスポンスから `isGuestVisitor` 削除
- `BulkUpdatePaymentUseCase.ts` — `getIsVisitor()` チェックに統一
- `RemoveParticipationUseCase.ts` — ローカル変数リネーム
- `AddGuestVisitorUseCase.ts` — `createUnregisteredVisitor()` 使用

**変更ファイル（FE）**:
- `api.ts` — `ParticipantItem` から `isGuestVisitor` 削除、`AddGuestVisitorRequest/Response` → `AddVisitorRequest/Response`
- `participationApi.ts` — `addGuestVisitor` → `addVisitor`
- `useParticipationQueries.ts` — `useAddGuestVisitor` → `useAddVisitor`
- `ActivityDetailPage.tsx` — ミューテーション・ダイアログ名をリネーム

### B. ConfirmPaymentUseCase バグ修正

**問題**: 未登録ビジター（`userId=null`）の支払い確認が空振りしていた。`ConfirmPaymentUseCase` が `userId` の null チェックで例外を投げ、`findLatestByScheduleAndUser` が呼べなかった。一括確認（BulkConfirmPayment）も同 UseCase をループ呼び出ししているため同様に失敗。

**修正内容**:
- `userId` がある場合: 従来通り `findLatestByScheduleAndUser(scheduleId, userId)` で Payment を検索
- `userId` が null の場合: `findByParticipationId(participationId)` でフォールバック検索
- `findByParticipationId` は既に `IPaymentRepository` / `PaymentRepositoryImpl` に存在していたため、新規メソッド追加は不要

**変更ファイル**: `backend/src/application/participation/usecase/ConfirmPaymentUseCase.ts`

### C. 支払い方法一括設定ダイアログ改善

**変更内容**:
- フィルタ対象を `isVisitor`（全ビジター）→ `isVisitor && !userId`（未登録ビジターのみ）に変更。登録済みビジターは自分で支払い方法を選べるため除外
- ダイアログ説明文を「ビジター（ゲスト）の支払い方法をまとめて設定します。」に変更
- ビジター名横のラベル（ビジター・ビジター（ゲスト））を削除（冗長のため）
- 注釈「※ビジター（ゲスト）は、アプリ登録を行っていないビジターです。」を追加

### D. BulkConfirmDialog（支払い一括確認）改善

- 参加者名横に「（ビジター・ゲスト）」/「（ビジター）」ラベルを追加し、ビジターの種類を視認可能に

### E. 参加者一覧テーブル UI 改善

| 項目                     | 変更前                     | 変更後                                                                      |
| ------------------------ | -------------------------- | --------------------------------------------------------------------------- |
| ビジター列（ゲスト表示） | なし                       | `✓` の下に `（ゲスト）`（7px）を縦積み表示。h-[21px] で行高さ統一           |
| テーブル高さ             | `max-h-64`（256px, 約7行） | `max-h-[331px]`（約10行表示）                                               |
| ヘッダー揃え             | 全列 text-center           | No.・参加者は text-left、他列は text-left（ヘッダー）/ text-center（内容）  |
| 参加取り消しアイコン     | `X`（❌）、名前直後に配置   | `UserMinus`（人型-マーク）、名前セル右端に配置。色 gray-400 → hover:red-500 |
| 参加取り消しラベル       | 「参加者を削除」           | 「参加を取り消す」                                                          |
