...
このドキュメントでは、TsudoCan が依存する外部サービスの設定手順を説明します。
...
2. **+ New Project** → プロジェクト名: `TsudoCan`
...
4. Products 作成:
   ...
   - Identifier: `tsudocan_web_monthly_320`
...
1. **Entitlements** → **+ New**
2. Identifier: `pro`
3. Description: `TsudoCan Pro features`

### 2-4. Product 設定

#### 月額サブスクリプション
1. **Products** → **+ New**
2. Identifier: `tsudocan_monthly_320`（App Store / Google Play の product ID と一致）
3. 価格: 320 JPY / 月
4. Entitlement: `pro` に紐付け

#### LIFETIME 買い切り
1. **Products** → **+ New**
2. Identifier: `tsudocan_lifetime`
...
2. **Packages**:
   - `$rc_monthly` → `tsudocan_monthly_320`
   - `$rc_lifetime` → `tsudocan_lifetime`
...
1. **サブスクリプション** → **+** → **サブスクリプショングループ** を作成
   - グループ名: `TsudoCan Pro`
2. **+ サブスクリプション**:
   - 製品 ID: `tsudocan_monthly_320`（RevenueCat の Product Identifier と一致）
   - 参照名: `月額プラン`
   - サブスクリプション期間: **1ヶ月**
3. **価格**:
   - 基本通貨: JPY
   - 価格: 320 円
4. **ローカリゼーション**:
   - 表示名: `TsudoCan Pro 月額プラン`
   - 説明: `チャット、DM、スタンプなどのプレミアム機能が使い放題`

#### LIFETIME（Non-consumable）
1. **App 内課金** → **+**
2. 種類: **非消費型**
3. 製品 ID: `tsudocan_lifetime`
4. 参照名: `LIFETIME プラン`
5. 価格: 5,980 円
6. ローカリゼーション:
   - 表示名: `TsudoCan Pro LIFETIME`
...
#### 月額サブスクリプション
1. **収益化** → **定期購入** → **定期購入を作成**
2. 商品 ID: `tsudocan_monthly_320`
3. 名前: `TsudoCan Pro 月額プラン`
4. **基本プラン** を追加:
   - 請求期間: **1ヶ月**
   - 価格: 320 円
...
2. 商品 ID: `tsudocan_lifetime`
3. 名前: `TsudoCan Pro LIFETIME`
4. 価格: 5,980 円
...
- [ ] Entitlement `pro` 作成
- [ ] Products 作成（`tsudocan_monthly_320` + `tsudocan_lifetime`）
