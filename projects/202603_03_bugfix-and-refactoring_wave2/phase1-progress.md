...
| タスク                         | 状態   | 備考                                                                        |
| ------------------------------ | ------ | --------------------------------------------------------------------------- |
| #13 タブスクロール位置保持     | ✅ 完了 | SectionTabs: forceMount + data-[state=inactive]:hidden                      |
| #16 FAB権限制御                | ✅ 完了 | useMyRole + isAdminOrAbove 条件追加                                         |
| #17 ブックマーク即時反映       | ✅ 完了 | announcementListKeys を楽観的更新 + invalidation に追加                     |
| #32 幹事データ不一致           | ✅ 完了 | ActivityForm の slice(0,5) → slice(0,10) に統一、max-h-60 に変更            |
| #41 キャンセル待ち条件付き表示 | ✅ 完了 | schedule.capacity != null 条件でラップ                                      |
| #42 参加費NULL不許容           | ✅ 完了 | Prisma NOT NULL DEFAULT 0 マイグレーション + 全レイヤー型統一               |
| #46 監査ログ論理名+ユーザー名  | ✅ 完了 | UseCase: User JOIN + actorDisplayName、フロント: フィールドラベルマッピング |
| #50 コミュニティ画像即時反映   | ✅ 完了 | #59 と同根、MyPage の onSuccess state reset で解決                          |
| #59 マイページ画像即時反映     | ✅ 完了 | onSuccess で displayName/biography/avatarPreview をリセット                 |
...
## 作業ログ
- 2025-07-15: Phase 1 全9件を実装完了。設計判断（#42: NOT NULL DEFAULT 0、#46: フロント側マッピング + UseCase User JOIN）を経て順次実装。フロント・バックエンド両方の型チェック通過確認済み。
