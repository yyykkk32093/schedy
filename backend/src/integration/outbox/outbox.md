...
### 1. 配信セマンティクス
- Outbox + Worker による配信は **At-least-once** 前提。
- 同一OutboxEventが **再送（retry）** により重複送信され得る。
- この重複は異常ではなく、**下流は idempotencyKey（Idempotency-Key / idempotencyKey）前提で冪等** とする。

### 2. Fan-out（1事実 → 複数配送指示）
- 1つの source event（IntegrationSource）から、**複数 OutboxEvent が生成され得る**。
- OutboxEventは「1配送先 = 1配送単位」とし、fan-outは **複数行（複数OutboxEvent）** で表現する。

### 3. routingKey 互換（audit.log の無期限併存）
- 互換期間として、`audit.log` と `user.lifecycle.audit` のような **複数 routingKey を無期限併存** させる。
- 併存期間中の二重送信/二重記録は **期待動作** とし、DLQ/監視上も「異常扱いしない」。
- 厳密な移行期限は設けず、個人PJとして設計検証を優先する。

### 4. Outbox確定と publish の責務分離
- Outboxは「購読の副作用」ではなく「事実確定プロセスの一部」。
- Outbox保存は **必ず UnitOfWork（同一トランザクション）内で確定** する。
- commit後に行うのは **publish（in-process副作用）だけ**。
      - Outbox永続化・外部連携は commit後には行わない。
      - commit後処理は失敗しても再実行可能な副作用に限定する。

### 5. IntegrationOutboxSubscriber の扱い
- `IntegrationOutboxSubscriber`（Auth/User など）は **Outbox永続化責務から撤退** する。
- 移行期の互換用途として残る可能性はあるが、最終的には削除前提。