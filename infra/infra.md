...
| -------------------------- | ---------------------- | -------------------- | ------ | ------------------------------------------ |
| **データベース**           | PostgreSQL 14 (Alpine) | `tsunaca-postgres`   | `5432` | アプリケーションの永続化層                 |
| **オブジェクトストレージ** | LocalStack (S3)        | `tsunaca-localstack` | `4566` | ファイルアップロード（ロゴ・カバー画像等） |
...
# バケット内のオブジェクト一覧
aws --endpoint-url=http://localhost:4566 s3 ls s3://tsunaca-local/
```

### 環境変数（backend/env/.env.local）

```dotenv
S3_BUCKET=tsunaca-local
...
| ボリューム名         | 用途                      |
| -------------------- | ------------------------- |
| `tsunaca-pgdata`     | PostgreSQL のデータ永続化 |
| `tsunaca-localstack` | LocalStack の状態永続化   |
